...
int main() {
  ...
  for (int timestamp{0}; timestamp < num_timestamps; ++timestamp) {
    ...
    sensor_readings.push_back(
        TimestampData{std::move(lidar_readings), camera_readings, imu_readings,
                      timestamp});
  }
  ...
  for (const auto &timestamp_data : sensor_readings) {
    ...
    const double lidar_average{
        lidar_sum / static_cast<double>(lidar_values.size())};
    lidar_average_history.push_back(lidar_average);
    const int obstacle_count{static_cast<int>(
        std::count_if(lidar_values.cbegin(), lidar_values.cend(),
    ...
    std::cout << "] Avg: " << lidar_average << " m, Obstacles: "
              << obstacle_count << ", Status: " << lidar_status << '\n';
    ...
    const auto [red_value, green_value, blue_value]{timestamp_data.camera_readings};
    const double channel_count{
        static_cast<double>(std::tuple_size<CameraData>::value)};
    const double brightness{
        static_cast<double>(red_value + green_value + blue_value) / channel_count};
    ...
    const bool imu_in_range{roll >= imu_min_rotation && roll <= imu_max_rotation &&
                            pitch >= imu_min_rotation &&
                            pitch <= imu_max_rotation &&
    ...
    std::cout << "IMU: RPY(" << roll << ", " << pitch << ", " << yaw
              << "), Total Rotation: " << total_rotation << " deg, Mode: "
  ...
  const double average_imu_rotation{
      total_readings["IMU"] > 0
          ? total_imu_rotation /
                static_cast<double>(total_readings["IMU"])
          : 0.0};
  ...
  std::cout << "DAY/NIGHT Count: " << day_mode_count << " / " << night_mode_count
            << '\n';
  ...
  if (!lidar_average_history.empty()) {
    ...
    const std::size_t lidar_min_index{static_cast<std::size_t>(std::distance(
        lidar_average_history.cbegin(), lidar_minmax.first))};
    const std::size_t lidar_max_index{static_cast<std::size_t>(std::distance(
        lidar_average_history.cbegin(), lidar_minmax.second))};
    std::cout << "LIDAR Average Range: " << *lidar_minmax.first << " m (Timestamp "
              << processed_timestamps[lidar_min_index] << ") to "
              << *lidar_minmax.second << " m (Timestamp "
              << processed_timestamps[lidar_max_index] << ")\n";
  }

  if (!camera_brightness_history.empty()) {
    const auto brightness_minmax{
        std::minmax_element(camera_brightness_history.cbegin(),
                            camera_brightness_history.cend())};
    const std::size_t brightness_min_index{static_cast<std::size_t>(std::distance(
        camera_brightness_history.cbegin(), brightness_minmax.first))};
    const std::size_t brightness_max_index{static_cast<std::size_t>(std::distance(
  ...
  if (!imu_rotation_history.empty()) {
    const auto rotation_minmax{
        std::minmax_element(imu_rotation_history.cbegin(),
                            imu_rotation_history.cend())};
    const std::size_t rotation_min_index{static_cast<std::size_t>(std::distance(
        imu_rotation_history.cbegin(), rotation_minmax.first))};
    const std::size_t rotation_max_index{static_cast<std::size_t>(std::distance(
        imu_rotation_history.cbegin(), rotation_minmax.second))};
